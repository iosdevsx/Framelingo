...
final class SubtitleTimecodeUtilsTests: XCTestCase {
    ...
    func testParseSRT() throws {
        let segments = try SRTSubtitleParser().parse(srtInput).segments
    ...
    func testRoundtripParseExport() throws {
        let parsed = try SRTSubtitleParser().parse(srtInput).segments
        let exported = subtitlesToSRT(parsed, mode: .original)
        XCTAssertEqual(exported, srtInput)
    }

    private var srtInput: String {
        """
        1
        00:00:01,000 --> 00:00:03,000
        Hello
    ...
    private var sampleSegments: [SubtitleSegment] {
        [
            SubtitleSegment(
                id: UUID(), index: 10, startMs: 1_000, endMs: 3_000,
                originalText: "Hello", translatedText: "Привет"
            ),
            SubtitleSegment(
                id: UUID(), index: 11, startMs: 4_000, endMs: 6_500,
                originalText: "Second line\ncontinues", translatedText: ""
            ),
        ]
    }
}
