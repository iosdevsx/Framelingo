...
struct CaptionLayoutCalculatorTests {
    @Test
    ...
    func testVerticalDefaultWrapsWithinEightyPercentAndThreeLines() throws {
        let layout = try #require(
            BurnedSubtitleLayoutHelper.makeVerticalCaptionLayout(
                ...
                settings: makeDefaultVerticalSubtitleStyle(),
    ...
    func testVerticalLayoutClampsToEveryPlatformSafeArea() throws {
        for platform in VerticalCaptionTestPlatform.allCases {
            var topStyle = makeDefaultVerticalSubtitleStyle()
            ...
            let minimumY = CGFloat(platform.topSafeAreaFraction) * 1_920
                + platform.configuration.safeAreaMargin
            #expect(topLayout.backgroundRect.minY >= minimumY)

            var bottomStyle = makeDefaultVerticalSubtitleStyle()
    ...
    func testVerticalLayoutClampsHorizontallyAndReportsNormalizedCenter() throws {
        for requestedX in [-1.0, 2.0] {
            ...
            let normalized = BurnedSubtitleLayoutHelper.normalizedPosition(
                for: layout,
                canvasSize: BurnedSubtitleLayoutHelper.verticalCanvasSize
            )
