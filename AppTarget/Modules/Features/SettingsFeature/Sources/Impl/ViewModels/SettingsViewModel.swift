...
final class SettingsViewModel: ObservableObject {
    ...
    var isVADModelInstalled: Bool {
        !settings.whisperVADModelPath.isEmpty
            && fileManager.fileExists(atPath: settings.whisperVADModelPath)
    }
    ...
    func installParakeetModels() async {
        ...
        do {
            try await parakeetModelStore.installModels { progress in
                await MainActor.run {
                    let fractionCompleted = progress.fractionCompleted ?? 0
                    self.parakeetInstallProgress = progress.fractionCompleted
                    self.parakeetInstallMessage = "\(progress.status) \(Int((fractionCompleted * 100).rounded()))%"
                }
            }
    ...
    func checkFFmpeg() async {
        ...
        do {
            let service = makeFFmpegService(settings)
